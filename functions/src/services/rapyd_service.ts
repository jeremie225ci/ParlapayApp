...
export class RapydService {
  private accessKey = ""
  private secretKey = ""
  private baseUrl = "https://sandboxapi.rapyd.net"
   ...
   */
  private calculateHMACSignature(toSign: string): string {
   ...
   */
  private generateSignature(httpMethod: string, path: string, salt: string, timestamp: number, body?: any): string {
    try {
  ...
  async getWalletBalance(walletId: string): Promise<any> {
    try {
      ...
      return {
        balance: response.data.accounts.reduce((total: number, account: any) => {
 ...
 */
 async transferFunds(
   ...
   */
  async addFunds(walletId: string, amount: number, currency: string, paymentMethod: any): Promise<any> {
    try {
   ...
   */
  async withdrawFunds(walletId: string, amount: number, currency: string, beneficiaryId: string): Promise<any> {
    try {
      const body = {
        ewallet: walletId,
        amount,
   ...
   */
  async getTransactionHistory(walletId: string, page: number, pageSize: number): Promise<any> {
    try {
      const response = await this.makeRequest(
        "GET",
        `/v1/user/${walletId}/transactions?page=${page}&page_size=${pageSize}`,
      );
   ...
   */
  async getIdentityStatus(identityId: string): Promise<any> {
    try {
