...
import { RapydService } from '../../services/rapyd_service';
...
export class CheckoutController {
  ...
  async initializeWalletBalances(req: Request, res: Response): Promise<void> {
    try {
  ...
  async getBalanceFromRapyd(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.params.userId || req.query.userId;
  ...
  async createCheckout(req: Request, res: Response): Promise<void> {
    try {
      ...
      const checkoutData = {
        ...
        complete_payment_url: null,  // usar null en lugar de URLs inválidas
        error_payment_url: null,     // usar null en lugar de URLs inválidas
        complete_checkout_url: null, // usar null en lugar de URLs inválidas
        cancel_checkout_url: null,   // usar null en lugar de URLs inválidas
...
async syncWalletBalance(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        ...
        const eurAccount = accounts.find((account: any) => account.currency === 'EUR');        
        if (!eurAccount) {
        ...
        if (walletDoc.exists) {
          ...
          await walletRef.update({
            balance: exactBalance, // Usar el balance exacto de Rapyd
            updatedAt: new Date(),
