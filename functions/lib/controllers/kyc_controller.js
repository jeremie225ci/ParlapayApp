    ...
    async processKYC(req, res) {
        try {
            ...
            if (error.message && error.message.includes('ERROR_CREATE_USER_EWALLET_REFERENCE_ID_ALREADY_EXISTS')) {
                ...
                // Otros errores
                res.status(500).json({
                    error: `Error de Rapyd: ${error.response ? error.response.status : ''} - ${error.response ? JSON.stringify(error.response.data) : error.message}`
                });
            }
        }
    }
...
//# sourceMappingURL=kyc_controller.js.map